...
@overload
def work(*, namespace: str | None = None) -> Callable[[C], C]: ...
...
def work(
    ...
    **Async Generator Support:**
    ...
    Example usage with named namespace for caching:
    ...
    .. code-block:: python
        ...
        @wool.work(namespace="cache")
        @lru_cache(maxsize=128)
        async def expensive_computation(n: int) -> int:
            # Result will be cached across invocations with same namespace
            return await some_expensive_operation(n)


        @wool.work(namespace="cache")
        async def another_cached_fn(x: int) -> int:
            # Shares state with expensive_computation
            return x * 2


        @wool.work(namespace=wool.WORKER)
        async def worker_level_task() -> str:
            # Uses worker-level globals
            return "worker"
    """
    ...
    # Handle @work vs @work() vs @work(namespace=...)
    if fn is not None:
        # Called as @work (bare decorator)
        return decorator(fn)
    # Called as @work() or @work(namespace=...)
    return decorator
