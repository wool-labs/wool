...
class InterceptorLike(Protocol):
    ...
    1. **Pre-dispatch**: Code before the first ``yield`` runs before the
       ...
       :class:`WorkTask` or ``None`` to use the original task.
    ...
    **Basic logging interceptor:**
    ...
    .. code-block:: python
        ...
        async def log_interceptor(task: WorkTask) -> AsyncGenerator:
    ...
    **Task modification interceptor:**
    ...
    .. code-block:: python
        ...
        async def rbac_interceptor(task: WorkTask) -> AsyncGenerator:
    ...
    **Stream filtering interceptor:**
    ...
    .. code-block:: python
        ...
        async def filter_interceptor(task: WorkTask) -> AsyncGenerator:
            response_stream = yield None
    ...
    **Error handling:**
    ...
    .. code-block:: python
        ...
        async def safe_interceptor(task: WorkTask) -> AsyncGenerator:
            try:
                response_stream = yield None
                async for event in response_stream:
                    yield event
    ...
    def __call__(
        self, task: WorkTask
    ) -> AsyncGenerator[WorkTask | None, AsyncIterator | None]: ...
...
class WoolInterceptor(AsyncServerInterceptor):
    ...
    async def intercept(
        ...
        task: WorkTask = cloudpickle.loads(request_or_iterator.task)
