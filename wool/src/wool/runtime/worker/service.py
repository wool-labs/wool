...
class WorkerService(protocol.WorkerServicer):
    ...
    async def _destroy_worker_loop(
        ...
        Clears the proxy and discovery-subscriber pools on the worker loop
        first, the one loop from which a `ResourcePool` bound to it can
        still be cleared. A clear that raises, times out, or is cancelled
        does not prevent the stop: a cancelled clear is retried while
        `_DRAIN_TIMEOUT` remains, and whatever a clear leaves cached is
        dropped when the pool next rebinds.
                ...
                for name, pool in pools:
                    ...
                    while True:
                        try:
                            ...
                            if cancels == 1:
                                _log.warning(
                                    f"Clearing the {name} pool was cancelled during "
                                    "worker-loop teardown; retrying while budget "
                                    "remains."
                                )
                            ...
                            if not (cancels and isinstance(exc, TimeoutError)):
                                ...
                                _log.warning(
                                    ...
                                    f"worker-loop teardown{cancelled}; continuing to "
                        ...
                        _log.warning(
                            ...
                            "complete within the budget; continuing to drain and "
                            "stop the loop."
                        )
                        break
                timed_out = False
                while not timed_out:
                    ...
                    def _on_done(task, pending=pending, drained=drained):
                        if not task.cancelled() and task.exception() is not None:
                            _log.debug(
                                "A task raised while the worker-loop teardown drain "
                                "cancelled it.",
