...
- A dedicated **worker event loop** runs on a daemon thread. Routines are scheduled here so that long-running work never blocks gRPC operations. The pools a routine draws on — its proxies, its discovery subscribers, and the gRPC channels a nested dispatch opens — keep their entries for this loop in a partition of their own and are finalized on it when it retires, because a `ResourcePool` serves each running loop through a private partition and only the owning loop can finalize what it cached; see `WorkerService` for the teardown order.
