...
The dispatch handler decodes the wire context into a `Snapshot` inside the session's parse phase. The worker driver task's first action is to install that snapshot — re-stamped so the worker-loop thread owns the chain — and the routine then runs under it. After each step the worker publishes the post-step snapshot onto the session so the handler can encode it for back-propagation; the handler reads it only after draining the worker, so the cross-thread read is race-free.
...
- **Initial-frame decode failure (request).** In non-strict mode each unreadable entry is dropped with a `wool.ContextDecodeWarning` and the routine runs under whatever partial snapshot decoded; an empty or entirely-unreadable frame leaves the worker context unarmed. In strict mode the promoted `wool.ContextDecodeWarning` is shipped via the `Nack` channel and the routine does not run.
