...
from wool.runtime.context.base import _context
...
from wool.runtime.context.guard import ChainContention
...
# Consulted by ``_ensure_task_factory_installed`` (and by anything
# else that needs to surface displacement loudly to user code) so a
# user-Wool entry point flagged on a displaced loop raises
# :class:`TaskFactoryDisplaced` regardless of whether mount is still
# the trigger path.
...
class _PendingSentinel:
    ...
    F19 — placed into ``_task_contexts`` by ``wool_factory`` *before*
...
# Guards (a) the read-modify-write in :func:`install_task_factory`
...
# :data:`_task_contexts` so the per-task registration/release sequence
...
class TaskFactoryDisplaced(WoolError):
    ...
    identity and trips :class:`~wool.ChainContention` on its first
    :class:`wool.ContextVar` access, or, when the parent has already
    finished, silently reuses the parent's chain identity.

    The displacement is detected reactively: Wool cannot intercept
    :meth:`loop.set_task_factory`, so it is noticed only on the next
    :class:`wool.ContextVar` access (or other path that arms a chain).
    ...
    (:class:`~wool.ContextDecodeWarning`) which can degrade gracefully
...
def _on_factory_collected(
    ...
    :data:`_displaced_loops` so the next user-Wool entry point
    consulting the flag raises :class:`TaskFactoryDisplaced` loudly to
    ...
    F21 — only mark displacement when ``loop.is_running()``. The
    finalize-during-shutdown window (between ``loop.close()``
    initiation and ``is_closed()`` returning True) previously
    flagged a closing loop as displaced and emitted a WARNING; demote
    that to a DEBUG since a non-running loop has no Wool API surface
    that could surface the flag anyway.
    """
    loop = loop_ref()
...
def _release(
    ...
    *coro* is the inner coroutine of a task whose coroutine the factory
    wrapped in :func:`_forked_scope`. By the time this done-callback
    ...
    unconditionally: :meth:`close` is a no-op on an exhausted coroutine,
    and on a never-started one it suppresses the "coroutine was never
    awaited" :class:`RuntimeWarning` that would otherwise leak at GC.
    *coro* is ``None`` for an unwrapped (unarmed) task, whose coroutine
    ...
    :func:`_on_factory_collected` finalize callback: if a third party
    ...
    if coro is not None:
        # F22 — close defensively. If ``coro.close()`` raises (e.g. a
...
async def _forked_scope(coro: Coroutine[Any, Any, T]) -> T:
    ...
    # The factory only wraps an armed child in this scope (see the
    # gate at the wool_factory definition), so by construction the
    # context here is non-None. Document the precondition; raise loud
    # if the gate-and-wrap drift ever breaks.
    assert context is not None, "_forked_scope invoked on unarmed child"
    # ``mount`` stamps owners and installs the task factory; fork
    # produces an owner-less, fresh-chain Context, so routing it
    # through mount is the canonical "make this fork live" step.
    context._fork().mount()
    return await coro


# F20 — Memoize per-factory `_is_wool_factory` outcomes, with built-in
...
# hot path (``_ensure_task_factory_installed`` fires from every
...
def _is_wool_factory(factory: Any) -> bool:
    ...
    idempotency check in :func:`install_task_factory` detect Wool even
    ...
    Memoized via a :class:`weakref.WeakKeyDictionary` and cycle-guarded
    by a per-walk ``seen`` set (F20).
    """
...
def install_task_factory(
    ...
    context onto a fresh chain. Idempotent — a subsequent call on a
    ...
    **Composed-factory contract** — When Wool composes with an existing
    ...
    :class:`contextvars.Context` itself so the chain-contention guard
    ...
    *depend* on ``name=`` being present. An inner factory written to
    ...
    :class:`TypeError` under composition and is unsupported — this
    ...
    :param loop:
        ...
        :func:`asyncio.get_running_loop`; calling with ``loop=None``
        outside a running loop raises :class:`RuntimeError`. The call
    ...
    with _lock:
        ...
        # F4 — clear the displacement flag atomically with the
        # re-install so a recovery path (``install_task_factory(loop)``
        # after the displacement was detected) self-heals. Without
        # this discard, ``_ensure_task_factory_installed`` continued to
        # short-circuit on ``if loop in _displaced_loops:
        # raise TaskFactoryDisplaced(...)`` even after the user
        # explicitly put Wool's factory back on top.
    ...
    def wool_factory(
        ...
        **kwargs: Any,
        ...
        if context is None:
            ...
            # state lives in the one ``_context`` binding the context
            ...
            # chain-owner guard (_assert_chain_owner) catches the
            ...
            #
            # F19 — reserve the slot with a ``_PENDING`` sentinel
            ...
            if owner is not None and (
                ...
                coro.close()
                # ``context[_context]`` is the armed Wool Context — by
                # the ``context_is_armed`` check above — so its
                # ``chain_id`` is the one being doubly-driven. Surface
                # it in the exception for diagnostics. F23 — also
                ...
                wool_ctx = context[_context]
                assert wool_ctx is not None  # gated by context_is_armed
                try:
                    current = asyncio.current_task()
                except RuntimeError:
                    current = None
                raise ChainContention(
                    chain_id=wool_ctx.chain_id,
                    kind="create_task",
        ...
        if not context_is_armed(context):
            ...
            try:
                ...
                # at GC, then re-raise. F19 — also clean up the
        ...
        # F19 — replace the ``_PENDING`` reservation we placed in the
...
def _ensure_task_factory_installed() -> None:
    ...
    :func:`install_task_factory` and this function add the loop to the
    set, so a loop set up by a direct :func:`install_task_factory`
    ...
    installed after Wool's — it raises :class:`TaskFactoryDisplaced`,
    ...
    paths: :data:`_displaced_loops` (set by
    :func:`_on_factory_collected`'s ``weakref.finalize`` callback the
    moment the loop drops Wool's factory reference, and by
    :func:`_release`'s done-callback backstop), the post-displacement
