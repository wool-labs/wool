...
def assert_chain_owner(chain: Chain | None) -> None:
    """Raise `ChainContention` if *chain* is entered by a thread
    or task that does not own it.
    ...
    A no-op for an unarmed context (*chain* is ``None``). On the
    ...
    :raises ChainContention:
        If *chain* is armed and the calling thread or running task is
        not its owner.
    """
    if chain is None:
        return
    current_thread = threading.get_ident()
    if chain.thread != current_thread:
        raise ChainContention(
            chain_id=chain.id,
            kind="thread",
            owning_thread=chain.thread,
            current_thread=current_thread,
        )
    owning_task = _resolve_owning_task(chain)
...
def _resolve_owning_task(context: Chain) -> asyncio.Future[Any] | None:
    ...
    against — the thread-owner check in `assert_chain_owner`
    ...
    :param context:
        ...
        Always non-``None`` — `assert_chain_owner` short-circuits
