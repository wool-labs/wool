...
# Keyed values are ``ContextVarManifest`` — the identity/data base — so a
# still-undeclared stub and a promoted ``ContextVar`` both register here.
# Weak values so unreferenced stubs are reclaimed when the embedding
# object graph drops them; ``lock`` above guards concurrent registration
# so two threads cannot observe an intermediate state.
var_registry: Final[
    weakref.WeakValueDictionary[tuple[str, str], ContextVarManifest[Any]]
