...
`to_thread` is the supported way to run blocking work on a
...
`wool.ContextVar` bindings without sharing chain identity.

The fork is the keystone: plain `asyncio.to_thread` copies the
caller's `contextvars.Context` verbatim, leaving the worker
thread on the *same* logical Wool chain. The first
`wool.ContextVar` access on the worker thread then trips the
chain-contention guard and raises `wool.ChainContention`.
`to_thread` here forks instead — a fresh chain UUID, owner-
stamped for the worker thread, with no merge-back path to the caller.

Lives in its own module so `wool.runtime.context.factory`
...
async def to_thread(
    ...
    **kwargs: Any,
    ...
    Mirrors `asyncio.to_thread` — runs *func* in the event
    ...
    the caller's `wool.ContextVar` bindings under a new chain
    UUID owned by the worker thread; mutations it makes do not
    propagate back to the caller (no merge-back).

    When called from an unarmed context (no `wool.ContextVar`
    ever set, no incoming chain manifest merged), the worker thread
    inherits the caller's plain `contextvars` context with no
    Wool chain — there is no chain to fork — and behaves exactly like
    `asyncio.to_thread` plus a no-op.

    This is the supported way to run Wool-aware work in another OS
    thread. Plain `asyncio.to_thread` from an armed context
    ...
    thread's first `wool.ContextVar` access then trips the
    chain-contention guard and raises
    `wool.ChainContention`. Use this function instead.

    See ``wool/src/wool/runtime/context/README.md`` for the model
    context.

    **Re-handoff is undefined behaviour.** A `to_thread` chain
    is owner-stamped for the worker thread and detached on purpose;
    handing the worker-side `contextvars.Context` back to the
    ...
    `wool.ContextVar` access — but the failure is not
    ...
    def _run() -> T:
        ...
        # copy_context() on the loop thread: Chain._fork stamps
        # ``thread`` with threading.get_ident(), so forking any
