...
class TestTaskCreationValuePropagationParity:
    @pytest.mark.asyncio
    async def test_child_observes_a_scoped_value(self, spawner, make_var):
...
class TestTaskCreationCopyOnFork:
    @pytest.mark.asyncio
    async def test_child_forks_a_fresh_chain(self, spawner):
        ...
        When:
            ...
            value and current_context().chain_id.
        Then:
        ...
        async def child() -> tuple[str, uuid.UUID]:
            ...
            return var.get(), context.chain_id
        ...
        assert observed[1] != parent.chain_id
        after = current_context()
        assert after is not None
        assert after.chain_id == parent.chain_id

    @pytest.mark.asyncio
    async def test_siblings_fork_distinct_chains(self, spawner):
    ...
    @pytest.mark.asyncio
    async def test_factory_is_dormant_with_an_unarmed_context(self, spawner):
    ...
    @pytest.mark.asyncio
    async def test_unarmed_factory_does_not_break_plain_contextvars(self, spawner):
        ...
        async def child() -> tuple[str, bool]:
            return var.get(), current_context() is None
...
class TestNestedTaskDepth:
    @pytest.mark.asyncio
    async def test_grandchild_forks_a_chain_distinct_from_child_and_parent(self):
        ...
        When:
            ...
            task, each reading current_context().chain_id.
        Then:
        ...
        async def child() -> tuple[uuid.UUID, uuid.UUID]:
            ...
            return context.chain_id, grandchild_chain

        # Act
        child_chain, grandchild_chain = await asyncio.create_task(child())

        # Assert
        assert len({parent.chain_id, child_chain, grandchild_chain}) == 3

    @pytest.mark.asyncio
    async def test_depth_0_value_is_visible_at_depth_3(self):
...
class TestGeneratorBasedCoroutine:
    @pytest.mark.asyncio
    async def test_generator_based_coroutine_propagates_a_value(self):
...
class TestTaskFactoryComposition:
    @pytest.mark.asyncio
    async def test_user_factory_installed_before_wool_still_forks(self):
    ...
    @pytest.mark.asyncio
    async def test_stdlib_value_propagates_under_composition(self):
    ...
    @pytest.mark.asyncio
    async def test_legacy_two_arg_inner_factory_raises_type_error(self):
    ...
    @pytest.mark.asyncio
    async def test_factory_installed_after_wool_emits_displacement_warning(self):
    ...
    @pytest.mark.asyncio
    async def test_install_task_factory_is_idempotent(self):
...
class TestChainContentionGuardBoundary:
    @pytest.mark.asyncio
    async def test_two_tasks_sharing_a_later_armed_context_trip_the_guard(self):
        """Test two tasks handed one context that is later armed trip the guard.

        Pins ``_assert_chain_owner``'s cross-task trigger boundary: the
    ...
    @pytest.mark.asyncio
    async def test_repassing_a_live_armed_context_is_rejected_up_front(self):
    ...
    @pytest.mark.asyncio
    async def test_callback_on_the_owning_thread_does_not_raise(self):
    ...
    @pytest.mark.asyncio
    async def test_offloaded_code_that_never_touches_a_var_does_not_raise(self):
...
class TestRunCoroutineThreadsafe:
    @pytest.mark.asyncio
    async def test_run_coroutine_threadsafe_propagates_a_value(self, make_var):
    ...
    @pytest.mark.asyncio
    async def test_run_coroutine_threadsafe_forks_a_fresh_chain(self):
        ...
        When:
            ...
            wool.ContextVar and reads current_context().chain_id.
        Then:
...
class TestSchedulingEdgeExceptionPropagation:
    @pytest.mark.asyncio
    async def test_lookup_error_in_a_gather_child_surfaces(self):
    ...
    @pytest.mark.asyncio
    async def test_lookup_error_in_a_done_callback_surfaces(self):
...
class TestRunInExecutorWithExplicitContext:
    @pytest.mark.asyncio
    async def test_run_in_executor_with_explicit_ctx_run(self):
        """Test loop.run_in_executor with an explicit ctx.run propagates a value.
    ...
    @pytest.mark.asyncio
    async def test_run_in_executor_with_ctx_run_on_an_unarmed_context(self):
