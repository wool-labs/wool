...
def _wool_owned(context: contextvars.Context) -> list[contextvars.ContextVar]:
    ...
    ``__wool`` name prefix (``__wool_context__`` for the chain
...
def _wool_owned_in_a_fresh_context(work) -> list[str]:
    """Run *work* in a brand-new Context and list the wool-owned variables.
...
class TestResetCrossContextRejection:
    @pytest.mark.asyncio
    async def test_reset_in_a_foreign_context_raises_value_error(self, make_var):
        """Test reset of a token in a different Context raises ValueError.
    ...
    @pytest.mark.asyncio
    async def test_reset_in_the_minting_context_succeeds(self, make_var):
        """Test reset of a token in its own Context restores the prior value.
...
class TestResetWrongVariableRejection:
    @pytest.mark.asyncio
    async def test_reset_with_a_token_from_another_variable_raises(self, make_var):
...
class TestTokenSingleUse:
    @pytest.mark.asyncio
    async def test_resetting_a_token_twice_raises(self, make_var):
...
class TestCopyContextPropagation:
    @pytest.mark.asyncio
    async def test_value_set_before_copy_is_visible_in_ctx_run(self, make_var):
    ...
    @pytest.mark.asyncio
    async def test_set_inside_ctx_run_does_not_leak_out(self, make_var):
...
class TestCopyContextWidth:
    def test_unarmed_context_carries_no_wool_variables(self):
    ...
    def test_armed_context_enumerates_one_plus_n_variables(self, n):
        ...
        assert wool_owned.count("__wool_context__") == 1
        backing = [name for name in wool_owned if name != "__wool_context__"]
        assert len(backing) == n


class TestAsyncGeneratorValuePropagation:
    @pytest.mark.asyncio
    async def test_value_is_visible_across_asend_athrow_and_aclose(self, make_var):
    ...
    @pytest.mark.asyncio
    async def test_asend_returns_the_next_yield_value(self, make_var):
