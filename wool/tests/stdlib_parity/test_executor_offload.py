...
- :meth:`loop.run_in_executor` — carries NO ``contextvars.Context`` of
  ...
  caller's stdlib variables nor a Wool chain. This holds for a
  thread-pool worker and for a freshly spawned process-pool worker; a
  fork-started worker is a memory clone and inherits the parent's
  ``contextvars`` regardless, so the process-pool test pins ``spawn``
  explicitly — the start method Wool's own ``WorkerProcess`` enforces.
- :func:`asyncio.to_thread` — copies the caller's
  ...
  then trips :class:`wool.ChainContention`.
...
class TestRunInExecutorParity:
    @pytest.mark.asyncio
    async def test_stdlib_contextvar_should_be_absent_in_run_in_executor(self):
    ...
    @pytest.mark.asyncio
    async def test_wool_contextvar_should_be_absent_in_run_in_executor(self):
        ...
        # Act
        observed = await loop.run_in_executor(None, lambda: wool.__chain__.get(None))

        # Assert
        assert observed is None

    @pytest.mark.asyncio
    async def test_run_in_executor_with_a_process_pool_should_carry_no_wool_context(
        self,
    ):
        ...
        When:
            ...
            ProcessPoolExecutor backed by a freshly spawned worker
            process that reads current_context.
        Then:
            It should observe no context in the worker process — a bare
            run_in_executor carries no Wool context of its own, so a
            spawned worker boots with no chain, matching the thread-pool
            path and stdlib.
        """
        # Arrange
        ...
        # Act
        # Pin ``spawn``: a spawned worker boots a fresh interpreter,
        # whereas a fork-started worker is a memory clone that inherits
        # the parent's contextvars (the Wool context and plain stdlib
        # vars alike), leaving no clean boundary to assert against.
        # Wool's own WorkerProcess enforces ``spawn`` for this reason.
...
class TestAsyncioToThreadParity:
    @pytest.mark.asyncio
    async def test_stdlib_contextvar_should_be_visible_in_asyncio_to_thread(self):
    ...
    @pytest.mark.asyncio
    async def test_plain_to_thread_should_raise_chain_contention_when_from_armed_context(
        self,
    ):
        ...
        Then:
            It should raise wool.ChainContention — the copied chain
        ...
        # Act & assert
        with pytest.raises(ChainContention, match="cannot be shared across OS threads"):
            await asyncio.to_thread(var.get)

    @pytest.mark.asyncio
    async def test_asyncio_to_thread_should_not_raise_when_unarmed_context(self):
...
class TestWoolToThreadParity:
    @pytest.mark.asyncio
    async def test_wool_to_thread_should_carry_value_without_guard(self):
    ...
    @pytest.mark.asyncio
    async def test_wool_to_thread_should_not_raise_when_unarmed_context(self):
    ...
    @pytest.mark.asyncio
    async def test_wool_to_thread_should_run_on_fresh_detached_chain(self):
        ...
        # Assert
        assert offloaded_chain != caller.id

    @pytest.mark.asyncio
    async def test_wool_to_thread_should_run_on_worker_thread(self):
    ...
    @pytest.mark.asyncio
    async def test_wool_to_thread_should_inherit_copy_of_caller_bindings(self):
