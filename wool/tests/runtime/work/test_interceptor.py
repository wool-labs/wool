...
def clear_interceptor_registry():
    ...
    from wool.runtime.work.interceptor import _registered_interceptors
...
@pytest.fixture
def sample_task():
    """Provide a mock WorkTask for testing."""
    task = MagicMock()
    task.id = "test-task-123"
    task.callable = MagicMock()
    task.args = ()
    task.kwargs = {}
    return task
...
def mock_grpc_context():
    ...
    context = MagicMock()
    context.cancel = MagicMock()
    return context


@pytest.fixture
def mock_request(sample_task):
...
class TestWoolInterceptorBridge:
    ...
    @pytest.mark.asyncio
    async def test_exits_early_with_no_interceptors(self, mock_grpc_context):
    ...
    @pytest.mark.asyncio
    async def test_bypasses_non_dispatch_methods(self, mock_grpc_context):
    ...
    async def test_interceptor_ordering_determinism_property(
        ...
        Given:
            Any WorkTask and 1-10 order-tracking interceptors
        When:
