...
class TestWorkerProxy:
    ...
    async def test_security_filter_with_credentials(
        ...
        """
        # Arrange
        mocker.patch.object(wp.wool, "__version__", "1.0.0")
    ...
    @pytest.mark.asyncio
    async def test__worker_sentinel_with_compatible_major_version(
        self,
        mock_proxy_session,
        local_major,
        local_minor,
        ...
        """Test workers with matching major version are accepted.

        Given:
            A proxy with version X.a.b and a worker with version
            X.c.d (same major version)
        When:
        ...
        mocker.patch.object(wp.wool, "__version__", local_version)
    ...
    async def test__worker_sentinel_with_unparseable_version(
        ...
        Given:
            ...
            version string has no parseable integer prefix
        When:
            The worker is discovered via static workers list
        Then:
            The worker is rejected from the load balancer context.
