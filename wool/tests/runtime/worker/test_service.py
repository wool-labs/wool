...
class TestWorkerService:
    ...
    async def test_dispatch_with_corrupt_context_under_strict_filter(
        ...
        Then:
            ...
            a typed :class:`wool.ContextDecodeError` aggregating the
            promoted warnings on ``.warnings``. The caller's existing
            ``except wool.ContextDecodeError`` clause matches without
        ...
        assert isinstance(raised, wool.ContextDecodeError)
        assert "Failed to deserialize" in str(raised)
        # Warnings aggregated on .warnings.
        assert len(raised.warnings) == 1
        assert isinstance(raised.warnings[0], wool.ContextDecodeWarning)

    @pytest.mark.asyncio
    async def test_dispatch_with_multiple_corrupt_vars_under_strict_filter(
        self, grpc_aio_stub, mock_worker_proxy_cache
    ):
        ...
        assert all(isinstance(w, wool.ContextDecodeWarning) for w in raised.warnings)
    ...
    async def test_dispatch_streaming_with_unpicklable_worker_mutation(
        ...
        Given:
            ...
            raises — the wool back-prop context
        ...
        Then:
            ...
            :class:`ContextDecodeWarning` is emitted on the worker —
            the context failure is ancillary state and does not
    ...
    async def test_dispatch_with_unpicklable_worker_mutation(
        ...
        Given:
            ...
            returning — the wool back-prop context
    ...
    async def test_dispatch_with_routine_raise_and_unpicklable_mutation(
        ...
        exception bare with the worker-side context failure
        ...
        Given:
            ...
            wool back-prop context's failure occur in the same
            done-callback
        When:
            The caller dispatches the routine
        Then:
            ...
            :class:`wool.ContextDecodeError` chained as
        ...
        # ``wool.ContextDecodeError`` aggregating the warning(s).
        cause = raised.__cause__
        assert isinstance(cause, wool.ContextDecodeError), (
    ...
    async def test_dispatch_with_attribute_rejecting_routine_exception_under_strict_mode(
        ...
        Pre-fix, ``e.__wool_context_warnings__ = warnings`` raised
        :class:`AttributeError` for classes that reject arbitrary
        writes, converting the routine's primary signal into a stray
        :class:`AttributeError` shipped to the caller. Post-fix, the
        side-channel attachment is best-effort: it falls through
        cleanly when the class rejects it. The routine exception
        type still ships.
        ...
        Given:
            ...
            wool context encode failure path) and then raises an
    ...
    async def test_dispatch_streaming_with_routine_raise_and_unpicklable_mutation(
        ...
        Given:
            ...
            the routine's failure and the back-prop context's
        ...
        Then:
            ...
            with the strict-mode :class:`wool.ContextDecodeError`
        ...
        # ``wool.ContextDecodeError`` aggregating the warning.
        cause = raised.__cause__
        assert isinstance(cause, wool.ContextDecodeError), (
            f"encode failure must appear on ``__cause__``; observed: "
            f"{type(cause).__name__}"
    ...
    @pytest.mark.asyncio
    async def test_dispatch_streaming_when_mount_raises(
        ...
        Given:
            ...
            invariant violation that :meth:`Context._update` raises
            on (``ValueError``)
        When:
        ...
        # mismatched chain id trips ``Context._update``'s parity check.
        ...
        # the worker's armed chain) — ``Context._update`` raises
    ...
    @pytest.mark.asyncio
    async def test_dispatch_chains_strict_mode_encode_failure_as_cause(
        self, grpc_aio_stub, mock_worker_proxy_cache, mocker: MockerFixture
    ):
        """Test the dispatch handler chains a strict-mode
        :class:`wool.ContextDecodeError` onto the routine's
        ...
        Given:
            ...
            :meth:`Context.to_protobuf` patched to raise a
        ...
        def encode_with_strict_failure(self, *args, **kwargs):
            raise wool.ContextDecodeError(
                wool.ContextDecodeWarning("var-1 unencodable"),
                wool.ContextDecodeWarning("var-2 unencodable"),
            )
    ...
    async def test_dispatch_with_caller_vars_for_coroutine_task(
        ...
        Then:
            ...
            the worker applied the wire-shipped context before running
            the task.
        """
        # Arrange
    ...
    async def test_dispatch_streaming_applies_var_updates_per_frame(
        ...
        # chain-parity check on ``Context._update``.
        caller_chain = uuid4()
    ...
    async def test_dispatch_ships_strict_mode_encode_error_when_routine_succeeds(
        ...
        single_warning = wool.ContextDecodeWarning("single bare peer")

        def encode_with_single_failure(self, *args, **kwargs):
            raise wool.ContextDecodeError(single_warning)
        ...
        assert terminal.HasField("exception")
        shipped = cloudpickle.loads(terminal.exception.dump)
        assert isinstance(shipped, wool.ContextDecodeError)
        assert len(shipped.warnings) == 1
        assert str(shipped.warnings[0]) == "single bare peer"

    @pytest.mark.asyncio
    async def test_dispatch_chains_strict_mode_encode_failure_on_async_generator_path(
        self, grpc_aio_stub, mock_worker_proxy_cache, mocker: MockerFixture
    ):
        ...
        Given:
            ...
            exception mid-stream, AND :meth:`Context.to_protobuf`
            patched to raise :class:`wool.ContextDecodeError` on
            the post-run encode.
        When:
            The dispatch RPC ships its terminal-exception Response
        ...
        Then:
            ...
            the :class:`wool.ContextDecodeError` chained on
            ``__cause__``.
        """
        from wool.runtime.context.base import Context as _Context
        ...
        # context) raise the strict-mode ContextDecodeError.
        call_count = {"n": 0}
        ...
        def encode_with_strict_failure(self, *args, **kwargs):
            ...
            raise wool.ContextDecodeError(
                wool.ContextDecodeWarning("agen-peer-1"),
                wool.ContextDecodeWarning("agen-peer-2"),
            )
    ...
    async def test_dispatch_drains_handler_on_terminal_exception_path_for_coroutine(
        ...
        """
        from wool.protocol.frame import Frame as _Frame
        from wool.protocol.frame import ResultResponseFrame as _ResultResponseFrame
