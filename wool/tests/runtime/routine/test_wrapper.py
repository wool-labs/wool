...
@routine(namespace="test_namespace")
...
assert namespace_func.__module__ == "runtime.routine.test_wrapper"


@routine(namespace="local_test")
...
assert local_exec_func.__module__ == "runtime.routine.test_wrapper"
...
class TestRoutineNamespaceParameter:
    """Tests for the @routine decorator with namespace parameter."""

    # WN-001: @routine without arguments defaults namespace=None
    def test_bare_decorator_defaults_namespace_none(self):
        """Test @routine bare decorator defaults namespace to None.

        Given:
            A function decorated with @routine (bare, no parentheses)
        When:
        ...
        # foo is decorated with @routine

        # Assert - verify the function is wrapped (indirectly)
        assert callable(foo)
        assert foo.__name__ == "foo"

    # WN-002: @routine() empty call defaults namespace=None
    def test_empty_call_defaults_namespace_none(self):
        """Test @routine() empty call defaults namespace to None.

        Given:
            A function decorated with @routine()
        When:
            The decorator is applied
        Then:
    ...
    # WN-003: @routine(namespace=None) explicit
    def test_explicit_namespace_none(self):
        """Test @routine(namespace=None) creates a properly wrapped function.

        Given:
            A function decorated with @routine(namespace=None)
        When:
            The decorator is applied
        Then:
            The function is properly wrapped
    ...
    # WN-004: @routine(namespace="name") explicit
    def test_explicit_namespace_string(self):
        """Test @routine(namespace="name") creates a properly wrapped function.

        Given:
            A function decorated with @routine(namespace="cache")
        When:
            The decorator is applied
        Then:
            The function is properly wrapped
    ...
    # WN-005: @routine(namespace=None) on async generator
    def test_namespace_none_on_async_generator(self):
        """Test @routine(namespace=None) works on async generators.

        Given:
            An async generator decorated with @routine(namespace=None)
        When:
            The decorator is applied
        Then:
            The function is properly wrapped
    ...
    # WN-006: @routine(namespace="name") on async generator
    def test_namespace_string_on_async_generator(self):
        """Test @routine(namespace="name") works on async generators.

        Given:
            An async generator decorated with @routine(namespace="shared")
        When:
            The decorator is applied
        Then:
            The function is properly wrapped
    ...
    # WN-007: @routine(namespace=None) on classmethod
    def test_namespace_none_on_classmethod(self):
        """Test @routine(namespace=None) works on classmethods.

        Given:
            A classmethod decorated with @routine(namespace=None)
        When:
            The decorator is applied
        Then:
    ...
    # WN-008: @routine(namespace="name") on staticmethod
    def test_namespace_string_on_staticmethod(self):
        """Test @routine(namespace="name") works on staticmethods.

        Given:
            A staticmethod decorated with @routine(namespace="shared")
        When:
            The decorator is applied
        Then:
            The method is properly wrapped
    ...
    async def test_dispatch_path_creates_task_with_namespace_none(
        ...
        Given:
            A function decorated with @routine (defaults to namespace=None)
        When:
    ...
    async def test_dispatch_path_creates_task_with_namespace_string(
        ...
        Given:
            A function decorated with @routine(namespace="test_namespace")
        When:
            The function is called in dispatch mode
        Then:
    ...
    async def test_local_execution_path_namespace_no_effect(self):
        ...
        Given:
            A function decorated with @routine(namespace="local_test")
        When:
    ...
    def test_unknown_parameter_raises_type_error(self):
        ...
        Given:
            A function decorated with @routine(unknown_param=True)
        When:
            The decorator is applied
        Then:
            TypeError is raised
        ...
        with pytest.raises(TypeError, match="unexpected keyword argument"):
            ...
            @routine(unknown_param=True)  # type: ignore
            async def unknown_param_func():
                return "result"

    # WN-012: @routine with WORKER sentinel
    def test_namespace_worker_sentinel(self):
        """Test @routine(namespace=WORKER) creates a properly wrapped function.

        Given:
            A function decorated with @routine(namespace=WORKER)
        When:
            The decorator is applied
        Then:
            The function is properly wrapped
    ...
    async def test_decorator_all_combinations(
        ...
        """Property test: @routine decorator works for all function/namespace combos.
        ...
        if function_type == "classmethod":
            ...
            class TestClass:
                @routine(namespace=namespace_value)
                @classmethod
