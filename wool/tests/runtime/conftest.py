...
@pytest.fixture
def stranded_loop():
    """Leave pool entries behind on a loop the test no longer drives.

    Yields a ``strand(coro, *, close=True)`` that drives the coroutine
    to completion on a fresh event loop and returns ``(loop, result)``.
    The loop is closed on the way out by default, or merely stopped
    when ``close=False``, so a test can distinguish a closed loop from
    one that stopped without closing, or resume it to let a stale timer
    fire. Every loop is closed at teardown.
    """
    loops = []
