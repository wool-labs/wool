"""Unit tests for the loop-partitioned resource pool.

Property tests here name their generated domain with ``_for_any_<domain>``
in place of the ``_when_<condition>`` clause the test guide specifies.
The phrase describes what the domain is rather than a single condition,
and it is used consistently across the suite; recorded as a deliberate
departure so it is not re-raised.
"""
...
#: Handles `_acquire` holds open, keyed by the pool, loop, and key
#: themselves, so a test can take and drop references non-lexically
#: through the public surface. Keyed by object rather than by ``id``:
#: ids are recycled — a few hundred sequential loops yield a handful of
#: distinct ones — so a Hypothesis example could otherwise find an
#: earlier example's handles in its own slot.
_handles: dict[tuple[Any, Any, Any], list[AsyncExitStack]] = {}
...
async def _acquire(pool, key):
    ...
    _handles.setdefault((pool, asyncio.get_running_loop(), key), []).append(stack)
    return obj


async def _release(pool, key):
    """Drop the most recent reference `_acquire` took on ``key`` on this loop."""
    slot = (pool, asyncio.get_running_loop(), key)
...
@pytest.fixture(autouse=True)
def _no_unawaited_coroutines(recwarn):
    """Fail a test that leaves one of the pool's coroutines unawaited.

    A ``never awaited`` warning means the pool regressed to building a
    coroutine ahead of the loop that would run it — see
    `Resource._hand_back` for why it does not. The warning surfaces at
    collection time, in whichever test happens to allocate next, so the
    collection is forced here while this test still owns it.
    """
    yield
    gc.collect()
    assert [
...
def _assert_one_stranded_report(caplog, stranded_loop, operation):
    """Run ``operation`` on a fresh loop over a stranded partition."""
...
def _assert_read_defers_stranded_report(caplog, stranded_loop, read):
    """Read through ``read`` on a fresh loop, then expire, over a stranded one."""
    pool = ResourcePool(factory=make_resource, ttl=60)
    stranded_loop(_acquire(pool, "key"))
...
def _loop_mixes(draw):
    """Draw a (running, closed) split of one to four loops.

    At least one loop stays running, since a closed loop cannot touch
    the pool.
    """
    total = draw(strategies.integers(1, 4))
    closed = draw(strategies.integers(0, total - 1))
...
class TestResourcePool:
    ...
    @example(ttl=0.0)
    @given(
        ttl=strategies.floats(
            min_value=0, max_value=60, allow_nan=False, allow_infinity=False
        )
    )
    async def test___init___should_defer_cleanup_for_any_positive_ttl(self, ttl):
        """Test the TTL decides whether the exit finalizes or defers.

        Given:
            A pool built with any non-negative TTL up to a minute,
            holding one entry under an open block
        When:
            The block is left, dropping the entry's last reference
        Then:
            It should finalize on the exit and leave the partition
            empty when the TTL is zero, and for every positive TTL
            leave the entry cached with a cleanup pending and its
            finalizer uncalled — the deferral turning on the sign of
            the TTL, not on its magnitude
        """
        # Arrange
        finalized = []
        pool = ResourcePool(factory=make_resource, finalizer=finalized.append, ttl=ttl)
        ...
        # Assert
        if ttl == 0:
            assert finalized == ["obj-test-key"]
            assert pool.stats == ResourcePool.Stats(0, 0, 0)
        else:
            assert finalized == []
            assert pool.stats == ResourcePool.Stats(1, 0, 1)

    @pytest.mark.asyncio
    async def test_get_should_cache_an_awaitable_object_as_is(self):
    ...
    def test_get_should_isolate_entries_when_two_loops_run_concurrently(
    ...
    def test_get_should_discard_the_partition_when_its_loop_closed(
        ...
        Then:
            It should discard the closed loop's partition -- the entry and
    ...
    def test_get_should_build_its_own_entry_when_another_loop_is_paused(
        self, mocker, caplog, stranded_loop
    ):
        """Test a second loop builds its own entry beside a paused loop's.
        ...
        Then:
            It should build a second resource for the acquiring loop's
            own partition and leave the paused loop's entry cached and
            unreported, since that loop can resume and finalize what it
            cached.
        """
        # Arrange
        factory = mocker.Mock(return_value="obj")
        pool = ResourcePool(factory=factory, ttl=60)
        paused, _ = stranded_loop(_acquire(pool, "key"), close=False)
        ...
        # Resumed: the entry the pause left behind is still its own, and
        # still referenced, which is what distinguishes retaining the
        # partition from discarding it.
        assert paused.run_until_complete(_pool_stats(pool)) == ResourcePool.Stats(
            total_entries=1, referenced_entries=1, pending_cleanup=0
        )

    def test_get_should_not_retain_a_loop_dropped_without_closing(self):
        """Test a pool holds nothing for a loop nobody closed.

        Given:
            A pool holding a referenced entry for a loop that was driven
            with run_until_complete and then dropped without being
            closed, so the discard's closed-loop test can never fire for
            it.
        When:
            The loop becomes unreachable and the collector runs.
        Then:
            It should let the loop be collected, taking its partition
            and the entries only it could have finalized with it.
        """
        # Arrange
        pool = ResourcePool(factory=make_resource, ttl=0)
        loop = asyncio.new_event_loop()

        async def hold():
            # The handle is dropped with the coroutine: what keeps the
            # partition alive here is the entry's reference count, not
            # anything the test still holds.
            await AsyncExitStack().enter_async_context(pool.get("key"))

        loop.run_until_complete(hold())
        watch = weakref.ref(loop)
    ...
    def test_get_should_retain_a_dropped_loop_while_a_ttl_timer_is_armed(self):
        """Test an armed TTL timer keeps its loop alive, as documented.

        Given:
            A pool holding an idle entry for a loop that was driven with
            run_until_complete and then dropped without being closed, so
            the entry's TTL timer is armed.
        When:
            The loop becomes unreachable and the collector runs.
        Then:
            It should still be retained, the timer handle referencing
            the loop that would have to fire it — the one case the weak
            registry cannot reclaim.
        """
        # Arrange
        pool = ResourcePool(factory=make_resource, ttl=60)
        loop = asyncio.new_event_loop()
    ...
    def test_get_should_not_warn_when_the_partition_was_cleared_before_closing(
        self, caplog, stranded_loop
    ):
        """Test a partition cleared by its own loop is discarded silently.
        ...
        Then:
            It should discard the empty partition without logging
    ...
    def test_get_should_warn_when_discarding_a_stranded_referenced_entry(
        self, caplog, stranded_loop
    ):
    ...
    def test_get_should_warn_when_discarding_a_stranded_idle_entry(
        self, caplog, stranded_loop
    ):
    ...
    def test_get_should_warn_once_per_partition_when_several_loops_stranded(
        self, caplog, background_loops
    ):
        """Test the discard drains every stale partition, reporting each.

        Given:
            A pool that cached one entry on each of three event loops
            running concurrently on their own threads, all of which
            then closed without clearing their partitions.
        When:
            The pool is used from a fresh event loop.
        Then:
    ...
    def test_get_should_discard_a_stranded_partition_when_its_loop_is_known(
        ...
        When:
            ...
            another key.
        Then:
            It should discard the closed loop's partition on that access
    ...
    def test_get_should_serve_each_loop_when_two_race_the_registry(
        self, background_loops
    ):
    ...
    def test_get_should_find_its_entry_when_its_loop_resumes(
        self, mocker, caplog, stranded_loop
    ):
    ...
    def test_get_should_name_the_pool_by_type_when_the_factory_has_no_qualname(
    ...
    @pytest.mark.asyncio
    async def test_get_should_leave_the_partition_empty_when_the_factory_raises(
        self, mocker
    ):
    ...
    def test_get_should_warn_once_when_a_partition_was_stranded(
        self, caplog, stranded_loop
    ):
        """Test acquiring on a fresh loop reports the stranded partition.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            A resource is acquired and released on a fresh loop.
        Then:
            It should log exactly one WARNING naming the referenced
            entry it discarded, an acquisition being a mutating access
            like the rest.
        """
        # Act & assert
        _assert_one_stranded_report(
            caplog, stranded_loop, lambda pool: _use_resource(pool.get("fresh"))
        )

    def test_get_should_report_only_the_pool_it_is_called_on(
        self, caplog, stranded_loop
    ):
        """Test one pool's discard never speaks for another's.
    ...
    def test_get_should_drop_the_loop_reference_when_it_discards_a_partition(self):
        """Test a discarded partition does not pin its loop forever.
    ...
    @pytest.mark.asyncio
    async def test_get_should_cancel_an_in_flight_cleanup_when_it_reacquires(
    ...
    @pytest.mark.asyncio
    async def test_get_should_report_nothing_when_it_cancels_a_fired_cleanup(
        self, caplog, mocker
    ):
    ...
    async def test_get_should_clear_the_retirement_when_it_reacquires(
        self, mocker, retire
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_should_leave_the_entry_unreferenced_when_cancelled_mid_cleanup(
        self, expiry_race_pool
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_should_cancel_the_pending_cleanup_when_it_reacquires(self):
        """Test reacquiring an entry inside its TTL cancels its cleanup.

        Given:
            A pool with a positive TTL holding one entry whose block
            has been left, so its cleanup is pending
        When:
            The entry is reacquired before the TTL fires
        Then:
            It should serve the same object, cancel the pending
            cleanup, and leave the entry cached with its finalizer
            never called
        """
        # Arrange
        mock_factory = Mock()
        ...
        # Act
        async with pool.get(key) as resource:
            reacquired = (resource, pool.stats)

        # Assert
        assert pending == ResourcePool.Stats(1, 0, 1)
        assert reacquired == (mock_resource, ResourcePool.Stats(1, 1, 0))
        mock_finalizer.assert_not_called()
        assert pool.stats.total_entries == 1
        assert pool.stats.referenced_entries == 0

    @pytest.mark.asyncio
    async def test_get_should_build_one_object_when_acquired_concurrently(
        self, counting_factory
    ):
        """Test concurrent acquisitions of one key share a single entry.

        Given:
            A pool with a TTL long enough to outlast the test, and ten
            tasks acquiring and releasing one key at once
        When:
            Every task has run to completion
        Then:
            It should have built the object once, served it to all ten,
            and left exactly one entry cached with its cleanup pending,
            the TTL deferring the finalizer rather than leaking it
        """
        # Arrange
        pool = ResourcePool(factory=counting_factory, ttl=60)
    ...
    @pytest.mark.asyncio
    @settings(max_examples=50, deadline=None)
    @given(factory=factory_functions(), finalizer=finalizer_functions())
    async def test_get_should_build_and_finalize_for_any_factory_shape(
        self, factory, finalizer
    ):
        """Test every supported factory and finalizer shape round-trips.

        Given:
            Any of the factory shapes the pool accepts — sync, async,
            lambda, async lambda, callable object and awaitable object
            — paired with any finalizer shape, including none, in a
            zero-TTL pool
        When:
            A key is acquired through a block and the block is left
        Then:
            It should serve the object that factory built, then
            finalize it exactly once on the exit and leave the
            partition empty, whichever shapes were drawn
        """
        # Arrange
        finalized = []

        async def recording_finalizer(obj):
            finalized.append(obj)
            if finalizer is not None:
                result = finalizer(obj)
                if inspect.isawaitable(result):
                    await result

        pool = ResourcePool(factory=factory, finalizer=recording_finalizer, ttl=0)

        # Act
        async with pool.get("test-key") as resource:
            held = (resource, pool.stats)

        # Assert
        assert held[0] is not None
        assert held[1] == ResourcePool.Stats(1, 1, 0)
        assert finalized == [held[0]]
        assert pool.stats == ResourcePool.Stats(0, 0, 0)
    ...
    def test_get_should_isolate_partitions_when_many_loops_run_concurrently(
    ...
    def test_get_should_build_one_object_per_loop_for_any_concurrency(
    ...
    def test_get_should_report_the_stranded_counts_for_any_mix(
        self, caplog, stranded_loop, referenced, idle
    ):
        """Test the discard's record counts whatever the loop stranded.
    ...
    def test_get_should_discard_only_the_closed_loops_for_any_mix(
        self, caplog, background_loops, mix
    ):
        """Test the discard is selective however the loops are mixed.
        ...
        When:
            ...
            another key.
        Then:
            It should log exactly one WARNING per closed loop and leave
            every running loop's entries, the discarding loop's included,
            cached.
        """
        # Arrange
        running_count, closed_count = mix
        pool = ResourcePool(factory=make_resource, ttl=60)
        running = [background_loops() for _ in range(running_count)]
        closed = [background_loops() for _ in range(closed_count)]
        try:
            for index, handle in enumerate(running + closed):
                handle.run(_acquire(pool, f"key-{index}"))
            ...
            # Act
            with caplog.at_level(logging.DEBUG, logger="wool.runtime.resourcepool"):
                running[0].run(_acquire(pool, "discarded-by"))

            # Assert
            records = _pool_records(caplog)
            assert [r.levelno for r in records] == [logging.WARNING] * closed_count
            assert all("1 referenced and 0 idle" in r.getMessage() for r in records)
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_decrement_the_reference_count(self):
        """Test leaving a resource's block decrements its reference count.

        Given:
            A long-TTL pool holding three cached entries, two of them
            referenced by an open block
        When:
            Each block is exited in turn
        Then:
            It should drop one reference per exit, leaving the pool's
            referenced count at zero with every entry still cached
        """
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_warn_when_the_spawned_cleanup_fails(self, caplog):
    ...
    async def test___aexit___should_evict_the_entry_when_the_finalizer_raises(
        ...
        Given:
            ...
            referenced — whose finalizer raises `asyncio.CancelledError` — a
        ...
        Then:
            The `asyncio.CancelledError` propagates, but the torn-down entry
    ...
    def test___aexit___should_fire_only_its_own_timer_when_its_loop_resumes(
        self, mocker, caplog, stranded_loop
    ):
    ...
    def test___aexit___should_finalize_when_its_loop_resumes_after_the_ttl(
        self, mocker, stranded_loop
    ):
    ...
    def test___aexit___should_not_report_a_cleanup_that_outlives_a_pause(
        self, caplog, stranded_loop
    ):
    ...
    def test___aexit___should_leave_no_pending_task_when_its_loop_closes_early(
        self, mocker
    ):
    ...
    async def test___aexit___should_finalize_a_retired_entry_when_it_is_the_last(
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_not_finalize_a_retired_entry_when_references_remain(
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_finish_finalizing_when_cancelled_mid_finalizer(
        self, mocker
    ):
        ...
        Then:
            It should raise `asyncio.CancelledError` to the releaser, let the
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_drop_the_reference_when_cancelled_on_the_lock(
        self,
    ):
        """Test a release cancelled while contending the lock still finalizes.

        Given:
            A pool of two entries whose sweep is parked inside the idle
            first entry's finalizer, holding the lock, while the second
            is retired but still referenced, so dropping its last
            reference must finalize it.
        When:
        ...
        # Retired up front: a release that only decrements never waits
        # for the lock, so only a finalizing one can be cancelled here.
        await pool.expire("b")
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_drop_the_reference_when_cancelled_behind_a_waiter(
        self,
    ):
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_not_spawn_a_task_when_the_entry_stays_cached(
        self, mocker
    ):
        """Test a release that only decrements spawns nothing and never yields.

        Given:
            A long-TTL pool holding one cached entry, so dropping a
            reference on it leaves the entry cached with its TTL armed.
        When:
            The entry is acquired and released through ``async with``.
        Then:
            It should create no task and schedule no callback, and leave
            the set of running tasks as it found it, the bookkeeping
            being synchronous on the path every dispatch takes.
        """
        # Arrange
        pool = ResourcePool(factory=make_resource, ttl=60)
        await _cache_idle_entry(pool, "key")
        loop = asyncio.get_running_loop()
        create_task = mocker.patch.object(loop, "create_task", wraps=loop.create_task)
        call_soon = mocker.patch.object(loop, "call_soon", wraps=loop.call_soon)
    ...
    def test___aexit___should_finalize_a_retired_entry_when_its_loop_ends(
        self, mocker, caplog
    ):
    ...
    async def test___aexit___should_finalize_before_returning_when_the_entry_is_retired(
        self, mocker, retire
    ):
    ...
    @pytest.mark.asyncio
    async def test___aexit___should_defer_the_finalizer_to_the_ttl(self):
        """Test releasing the last reference defers the finalizer to the TTL.

        Given:
            A pool with a positive TTL holding one entry under an open
            block
        When:
            The block is exited and the TTL is allowed to elapse
        Then:
            It should keep the entry cached with a cleanup pending and
            the finalizer uncalled at the exit, then evict it and call
            the finalizer once the TTL fires
        """
        # Arrange
        mock_factory = Mock()
        mock_finalizer = AsyncMock()
        ...
        async with pool.get(key) as resource:
            held = (resource, pool.stats)
        deferred = pool.stats
        finalized_at_exit = mock_finalizer.call_count
        await _poll_until(lambda: key not in pool.pending_cleanup)

        # Assert
        assert held == (mock_resource, ResourcePool.Stats(1, 1, 0))
        assert deferred == ResourcePool.Stats(1, 0, 1)
        assert finalized_at_exit == 0
        assert pool.stats.total_entries == 0
    ...
    async def test___aexit___should_contain_and_report_a_failing_finalizer(
        self, ttl, retire_first, caplog
    ):
        """Test a finalizer's exception is contained, reported and evicting.
        ...
        When:
            The block holding the entry is left, running the finalizer
        Then:
            It should return without raising, evict the entry anyway,
            and report one warning naming the pool and the key
        """
        # Arrange
        mock_factory = Mock()
        ...
        records = _pool_records(caplog)
        assert len(records) == 1
        assert "finalizer failed" in records[0].getMessage()
        assert f"{key!r}" in records[0].getMessage()
    ...
    def test___aexit___should_keep_each_loops_bookkeeping_independent(
    ...
    async def test___aexit___should_maintain_bookkeeping_invariants(self, operations):
    ...
    def test_expire_should_warn_once_when_a_partition_was_stranded(
        self, caplog, stranded_loop
    ):
        """Test expiring on a fresh loop reports the stranded partition.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            `expire` runs on a fresh loop.
        Then:
            It should log exactly one WARNING naming the referenced
            entry it discarded.
        """
        # Act & assert
        _assert_one_stranded_report(caplog, stranded_loop, lambda pool: pool.expire("x"))
    ...
    async def test_expire_should_cancel_an_in_flight_cleanup_when_the_ttl_fired(
    ...
    async def test_expire_should_finalize_only_the_idle_entry_when_one_is_held(
        self, mocker, retire
    ):
    ...
    async def test_expire_should_not_raise_when_nothing_is_cached(self, mocker, retire):
    ...
    def test_expire_all_should_warn_once_when_a_partition_was_stranded(
        self, caplog, stranded_loop
    ):
        """Test expiring everything on a fresh loop reports the stranded one.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            `expire_all` runs on a fresh loop.
        Then:
            It should log exactly one WARNING naming the referenced
            entry it discarded.
        """
        # Act & assert
        _assert_one_stranded_report(
            caplog, stranded_loop, lambda pool: pool.expire_all()
        )
    ...
    @pytest.mark.asyncio
    async def test_expire_all_should_not_uncancel_a_cancellation_a_finalizer_took(
        self,
    ):
        """Test a cancellation a finalizer swallowed is left on the task.

        Given:
            A long-TTL pool of two idle entries whose first finalizer
            parks until the sweeping task is cancelled and then
            suppresses the cancellation it receives.
        When:
            expire_all() runs in a task that is cancelled while that
            finalizer is parked.
        Then:
            It should leave the task's cancellation count at one: the
            sweep never caught that cancellation, so uncancelling for it
            would discard a caller's bid — an enclosing timeout's, say —
            that nothing has honoured.
        """
        # Arrange
        parked = asyncio.Event()
        counts = []
    ...
    def test_clear_should_warn_once_when_a_partition_was_stranded(
        self, caplog, stranded_loop
    ):
        """Test clearing on a fresh loop reports the stranded partition.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            `clear` runs on a fresh loop.
        Then:
            It should log exactly one WARNING naming the referenced
            entry it discarded.
        """
        # Act & assert
        _assert_one_stranded_report(caplog, stranded_loop, lambda pool: pool.clear())
    ...
    @pytest.mark.asyncio
    async def test_clear_should_finish_a_release_that_was_finalizing(self):
        """Test a clear drives a finalizing release it finds in flight.

        Given:
            A zero-TTL pool whose last reference is being dropped, so
            the release that finalizes the entry is in flight on a task
            of its own that has not yet taken the lock.
        When:
            The loop clears the pool.
        Then:
            It should return with that release finished rather than
            still pending, so a loop that stops after clearing has no
            release of the pool's left to destroy.
        """
        # Arrange
        finalized = []
        pool = ResourcePool(factory=make_resource, finalizer=finalized.append, ttl=0)
        await _acquire(pool, "key")
        release = asyncio.ensure_future(_release(pool, "key"))
        # One tick: enough for the release to hand its finalization to a
        # task, not enough for that task to run.
        await asyncio.sleep(0)
        ...
        # Assert
        assert release.done()
        assert finalized == ["obj-key"]
        assert pool.stats == ResourcePool.Stats(0, 0, 0)

    @pytest.mark.asyncio
    async def test_clear_should_raise_when_the_deadline_has_already_elapsed(self):
        """Test a clear handed a spent deadline reaches no key at all.

        Given:
            A long-TTL pool holding two idle entries, and a deadline
            already behind the running loop's clock.
        When:
            The pool is cleared with that deadline.
        Then:
            It should raise TimeoutError naming the entries it did not
            reach and run no finalizer, the deadline being checked
            before each key rather than after.
        """
        # Arrange
        finalized = []
        pool = ResourcePool(factory=make_resource, finalizer=finalized.append, ttl=60)
        await _cache_idle_entry(pool, "a")
        await _cache_idle_entry(pool, "b")

        # Act & assert
        with pytest.raises(TimeoutError, match="2 entries it did not reach"):
            await pool.clear(deadline=asyncio.get_running_loop().time() - 1)

        assert finalized == []
        assert pool.stats.total_entries == 2

    @pytest.mark.asyncio
    async def test_clear_should_leave_the_remaining_entries_cached_when_it_elapses(
        self, caplog
    ):
        """Test a deadline reached mid-sweep stops the clear where it stands.

        Given:
            A long-TTL pool holding three idle entries whose finalizer
            outlives the deadline on the first key it reaches.
        When:
            The pool is cleared with a deadline a fraction of a second
            out.
        Then:
            It should cancel the finalizer it was running, report that
            it was interrupted, raise TimeoutError, and leave the keys
            it never reached cached rather than finalizing them
            unbounded.
        """
        # Arrange
        entered = []

        async def outlive_the_deadline(obj):
            entered.append(obj)
            await asyncio.sleep(5)

        pool = ResourcePool(
            factory=make_resource, finalizer=outlive_the_deadline, ttl=60
        )
        for key in ("a", "b", "c"):
            await _cache_idle_entry(pool, key)
        ...
        with caplog.at_level(logging.WARNING, logger="wool.runtime.resourcepool"):
            with pytest.raises(TimeoutError):
                await pool.clear(deadline=asyncio.get_running_loop().time() + 0.05)

        assert len(entered) == 1
        assert pool.stats.total_entries == 2
        # The record only the per-key cancellation produces: waiting the
        # finalizer out and stopping before the next key would leave the
        # same counts behind.
        records = _pool_records(caplog)
        assert len(records) == 1
        assert "interrupted by a cancellation" in records[0].getMessage()

    @pytest.mark.asyncio
    async def test_clear_should_finalize_everything_when_the_deadline_is_ample(self):
        """Test a deadline the sweep finishes inside changes nothing.

        Given:
            A long-TTL pool holding three idle entries and a deadline
            far enough out that every finalizer completes.
        When:
            The pool is cleared with that deadline.
        Then:
            It should finalize every entry and return without raising,
            a deadline being a bound rather than an instruction.
        """
        # Arrange
        finalized = []
        pool = ResourcePool(factory=make_resource, finalizer=finalized.append, ttl=60)
        ...
        # Act
        await pool.clear(deadline=asyncio.get_running_loop().time() + 30)

        # Assert
        assert sorted(finalized) == ["obj-a", "obj-b", "obj-c"]
        assert pool.stats == ResourcePool.Stats(0, 0, 0)

    @pytest.mark.asyncio
    async def test_clear_should_leave_a_finalizing_release_running_when_it_elapses(
        self, caplog
    ):
        """Test a spent deadline abandons the pre-sweep wait rather than it.

        Given:
            A zero-TTL pool whose last reference is being dropped, so a
            release that finalizes slowly is in flight on a task of its
            own, and a deadline a fraction of a second out.
        When:
            The pool is cleared with that deadline.
        Then:
            It should return from the wait with that release still
            running, neither cancelling nor reporting it, which is the
            bound the deadline places on waiting rather than on the
            release.
        """
        # Arrange
        finalized = []

        async def finalize_slowly(obj):
            await asyncio.sleep(0.3)
            finalized.append(obj)

        pool = ResourcePool(factory=make_resource, finalizer=finalize_slowly, ttl=0)
        await _acquire(pool, "key")
        release = asyncio.ensure_future(_release(pool, "key"))
        await asyncio.sleep(0)
        ...
        with caplog.at_level(logging.DEBUG, logger="wool.runtime.resourcepool"):
            await pool.clear(deadline=asyncio.get_running_loop().time() + 0.05)
            still_running = not release.done()
    ...
    @settings(max_examples=25, deadline=None)
    @given(
        entry_count=strategies.integers(1, 5),
        offset=strategies.floats(min_value=-1.0, max_value=30.0),
    )
    async def test_clear_should_finalize_everything_for_any_sufficient_deadline(
        self, entry_count, offset
    ):
        """Test the deadline decides only whether the sweep gets to finish.

        Given:
            A long-TTL pool holding between one and five idle entries
            whose finalizers return at once, and any deadline offset
            from a second in the past to thirty seconds out.
        When:
            The pool is cleared with that deadline.
        Then:
            It should either finalize every entry and return, or raise
            TimeoutError having finalized a prefix of them, and in
            either case leave exactly the entries it finalized out of
            the partition — a deadline never loses an entry it closed
            nor closes one it reports as unreached.
        """
        # Arrange
        finalized = []
        pool = ResourcePool(factory=make_resource, finalizer=finalized.append, ttl=60)
        ...
        # Act
        try:
            await pool.clear(deadline=asyncio.get_running_loop().time() + offset)
        except TimeoutError:
            timed_out = True
        else:
            timed_out = False
        ...
        # Assert
        assert set(finalized) <= {f"obj-{key}" for key in keys}
        assert pool.stats.total_entries == entry_count - len(finalized)
        assert timed_out == (len(finalized) < entry_count)
    ...
    def test_stats_should_not_discard_a_stranded_partition(self, caplog, stranded_loop):
        """Test a stats read leaves a stranded partition for a mutating access.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            `stats` is read on a fresh loop, and the pool is then
            expired on that loop.
        Then:
            It should log nothing for the read and report the stranded
            partition once, from the expire that follows it.
        """
        # Act & assert
        _assert_read_defers_stranded_report(
            caplog, stranded_loop, lambda pool: pool.stats
        )
    ...
    async def test_stats_should_return_accurate_counts(self):
        """Test the counters track entries, references and pending cleanups.

        Given:
            A pool with a positive TTL and three keys, held under three
            nested blocks
        When:
            The counters are read empty, with all three blocks open,
            and again once all three have been left
        Then:
            It should report zeros for the empty pool, three entries
            all referenced with nothing pending while the blocks are
            open, and three entries none referenced with three
            cleanups pending afterwards, so every counter is observed
            both at zero and above it
        """
        # Arrange
        mock_factory = Mock()
        mock_finalizer = AsyncMock()
        ...
        async with pool.get("key1"):
            async with pool.get("key2"):
                async with pool.get("key3"):
                    held = pool.stats
        released = pool.stats

        # Assert
        assert empty == ResourcePool.Stats(0, 0, 0)
        assert held == ResourcePool.Stats(3, 3, 0)
        assert released == ResourcePool.Stats(3, 0, 3)
    ...
    def test_pending_cleanup_should_not_discard_a_stranded_partition(
        self, caplog, stranded_loop
    ):
        """Test a pending-cleanup read leaves a stranded partition alone.

        Given:
            A pool holding one referenced entry stranded on a closed
            loop.
        When:
            `pending_cleanup` is read on a fresh loop, and the pool is
            then expired on that loop.
        Then:
            It should log nothing for the read and report the stranded
            partition once, from the expire that follows it.
        """
        # Act & assert
        _assert_read_defers_stranded_report(
            caplog, stranded_loop, lambda pool: pool.pending_cleanup
        )
...
class TestResource:
    ...
    @pytest.mark.asyncio
    async def test___aenter___should_acquire_and_release_around_the_block(self, mocker):
    ...
    async def test___aexit___should_finalize_when_the_entry_is_retired_in_the_body(
    ...
    def test___aexit___should_build_no_coroutine_when_the_paused_loop_never_runs(
        self, recwarn, stranded_loop
    ):
        """Test a hand-back to a loop that never resumes reports nothing.

        Given:
            A resource acquired on a loop that has stopped without
            closing, whose exit is attempted from another loop, so the
            release is handed back to it.
        When:
            The paused loop is closed without ever resuming.
        Then:
            It should leave no coroutine unawaited, the hand-back having
            scheduled a callable rather than something a loop that never
            runs it would have to report.
        """
        # Arrange
        pool = ResourcePool(factory=make_resource, ttl=60)
        stack = AsyncExitStack()
        ...
        paused, _ = stranded_loop(hold(), close=False)

        # Act
        with pytest.raises(RuntimeError, match="handed to the acquiring loop"):
    ...
    def test___aexit___should_report_a_stranded_reference_when_its_loop_closed(
        self, mocker, background_loops
    ):
        """Test a cross-loop exit after the acquiring loop closed says so.

        Given:
            A resource entered on an event loop running on another
            thread, that loop then closed without clearing its
            partition.
        When:
            The resource is exited on a second loop.
        Then:
            It should raise RuntimeError saying the acquiring loop is
            closed and the reference is stranded, which is the half of
            the diagnostic that tells an operator a leak from a
            deferral.
        """
        # Arrange
        finalizer = mocker.AsyncMock()
        pool = ResourcePool(factory=make_resource, finalizer=finalizer, ttl=60)
        acquiring = background_loops()
        resource = pool.get("key")
        stack = AsyncExitStack()
        acquiring.run(stack.enter_async_context(resource))
        acquiring.close()
        ...
        assert str(raised.value) == (
            "ResourcePool(make_resource) cannot release key 'key' on a loop "
            "other than the one that acquired it; the acquiring loop is "
            "closed, so the reference is stranded"
        )
        finalizer.assert_not_awaited()

    def test___aexit___should_warn_when_the_handed_back_release_fails(
        self, background_loops, caplog
    ):
        """Test a release the acquiring loop ran and failed is reported.

        Given:
            A zero-TTL pool whose finalizer raises a `BaseException`
            the pool does not contain, and a resource entered on an
            event loop running on another thread.
        When:
            The resource is exited on a second loop, handing the
            release back to the loop that took it.
        Then:
            It should report one warning naming the pool and the key,
            the hand-back running on a task with no caller left to
            propagate the failure to.
        """
        ...
        with caplog.at_level(logging.WARNING, logger="wool.runtime.resourcepool"):
            with pytest.raises(RuntimeError):
                asyncio.run(_close_stack_elsewhere(resource))
            acquiring.run(_poll_until(lambda: bool(_pool_records(caplog))))

        # Assert
        records = _pool_records(caplog)
        assert len(records) == 1
        message = records[0].getMessage()
        assert "handed to its acquiring loop failed" in message
