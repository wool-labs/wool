...
# armed chain directly through the public ``wool.__chain__`` context var
# (rather than ``Chain.mount``, which restamps the owner), verifying the
# adoption path on first `ContextVar.set`.
...
class TestContextVar:
    def test___new___should_raise_lookup_error_on_get_when_name_only(self):
    ...
    def test___new___should_return_default_on_get_when_default_given(self):
    ...
    def test___new___should_infer_namespace_from_caller_package(self):
    ...
    def test___new___should_use_explicit_namespace_when_provided(self):
    ...
    def test___new___should_raise_collision_when_key_already_registered(self):
    ...
    def test___new___should_raise_collision_when_duplicate_key_regardless_of_default(
        self, name, default_a, default_b
    ):
    ...
    def test___new___should_register_both_when_same_name_in_different_namespace(self):
    ...
    def test___new___should_seed_supplied_default_when_promoting_stub(self):
        ...
        pb = protocol.ChainManifest(id=uuid.uuid4().hex)
        pb.vars.add(
        ...
        _decoded = ChainManifest.from_protobuf(pb, serializer=wool.__serializer__)  # noqa: F841
    ...
    def test_contextvar_should_declare_empty_slots(self):
        """Test ContextVar declares no instance slots of its own.

        Given:
            The ContextVar type, whose declared instances must reuse the
            exact storage a pre-declaration placeholder already holds.
        When:
            Its own __slots__ are inspected.
        Then:
            They should be empty — the identity and data fields live on
            the base, and an empty subclass layout is what lets a
            placeholder be upgraded to a full variable in place when the
            variable is declared.
        """
        # Act & assert
        assert ContextVar.__slots__ == ()

    def test_declaration_should_keep_placeholder_object_and_value_in_place(self):
        """Test declaring a wire-seeded variable keeps its object and value.

        Given:
            A variable whose value was applied to its placeholder backing
            before the receiver declared it — modelling a propagated
            value that arrived ahead of the declaration.
        When:
            The receiver declares the variable for that key.
        Then:
            Declaration should hand back the same object the placeholder
            occupied, now usable, and reading it should yield the value
            applied before declaration — the upgrade happens in place, so
            nothing the placeholder held is lost.
        """
        # Arrange
        namespace = uuid.uuid4().hex
        name = "inplace_promote"
        placeholder = resolve_stub((namespace, name))
        placeholder_id = id(placeholder)
        placeholder._backing.set("pre-declaration")

        # Act
        var = ContextVar(name, namespace=namespace)

        # Assert
        assert id(var) == placeholder_id
        assert var.get() == "pre-declaration"
    ...
    def test___new___should_raise_type_error_when_name_not_str(self, non_str_name):
    ...
    def test_repr_should_include_namespace_and_name(self):
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_get_should_resolve_the_default_ladder(
    ...
    def test_get_should_return_none_when_default_is_none(self):
    ...
    def test_get_should_raise_type_error_when_more_than_one_positional_argument(self):
    ...
    def test_get_should_return_set_value(self):
    ...
    def test_get_should_not_arm_an_unarmed_context(self):
    ...
    def test_set_should_arm_an_unarmed_context(self):
        ...
        Then:
            current_context should return a Chain.
        """
        # Arrange
    ...
    def test_set_should_return_token(self):
    ...
    def test_set_should_rebuild_the_context_on_each_call(self):
        ...
        with scoped_context():
            var.set("first")
            first_context = wool.__chain__.get(None)
            var.set("second")
            second_context = wool.__chain__.get(None)
    ...
    def test_set_should_clear_a_prior_reset_signal(self):
        ...
        with scoped_context():
            token = var.set("first")
            var.reset(token)
            after_reset = wool.__chain__.get(None)
            assert after_reset is not None
            assert (var.namespace, var.name) in after_reset.resets
            var.set("second")
            after_set = wool.__chain__.get(None)
            assert after_set is not None
            assert (var.namespace, var.name) not in after_set.resets

    @given(payload=st.text() | st.integers() | st.lists(st.integers()))
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_set_should_round_trip_with_arbitrary_value(self, payload):
    ...
    @pytest.mark.asyncio
    async def test_set_should_adopt_an_armed_chain_with_no_live_owning_task(self):
        ...
        Then:
            The active context's owning task (``Chain.task``) should
            become the running task — a new runner adopts an ownerless
            armed chain so a later concurrent task still fails loud via
            the owning-task guard.
        """
        ...
        with scoped_context():
            # An armed chain with task=None — the shape of a context
            # decoded off the wire before a worker mounts it.
            wool.__chain__.set(
                Chain(
                    id=uuid.uuid4(),
            ...
            try:
                ...
                owning_task = await task
            finally:
                loop.set_task_factory(None)

            # Assert — the running task adopted ownership.
            assert owning_task is task

    def test_set_should_propagate_value_into_seeded_contextvars_context(self):
    ...
    def test_reset_should_restore_default_when_first_set_token(self):
    ...
    def test_reset_should_restore_outer_value_when_nested_set_token(self):
    ...
    def test_reset_should_raise_runtime_error_when_token_already_used(self):
    ...
    def test_reset_should_raise_value_error_when_token_for_different_var(self):
    ...
    def test_reset_should_restore_unset_state_when_old_value_missing(self):
    ...
    def test_reset_should_raise_value_error_when_token_reset_in_copied_context(self):
    ...
    def test_reset_should_not_raise_when_chain_unarmed_after_native_reset(self, mocker):
        ...
        Given:
            A ContextVar armed by a set that captures a Token, with the
            active Wool context patched to report the chain unarmed.
        When:
            reset(token) is called — the native backing reset succeeds,
            then the Wool-bookkeeping branch observes no armed chain.
        Then:
            reset should return without raising — the defensive no-op
            guarding the post-native-reset path that is otherwise
            unreachable, since stdlib's native reset would itself have
            raised before an armed chain could vanish.
        """
        # Arrange
        var = ContextVar(_unique("reset_unarmed_noop"), default="d")

        # Act & assert
        unset = object()
        with scoped_context():
            token = var.set("x")
            mocker.patch.object(wool, "__chain__").get.return_value = None
            var.reset(token)
            assert var.get(unset) is unset

    def test_pickle_should_roundtrip_to_same_instance_when_var_registered(self):
    ...
    def test_pickle_should_not_write_to_receiver_context_on_roundtrip(self):
        ...
        def _receive() -> None:
            ...
            observed.append(wool.__chain__.get(None))
    ...
    def test_pickle_should_seed_default_into_stub_when_reloaded_after_wire_decode(self):
        """Test loading a cloudpickled ContextVar with a default seeds an existing stub.

        Given:
            A receiver process where a wire protocol.ChainManifest carrying a
        ...
        # Bound to keep the decoded manifest alive so its stubs anchor
    ...
    def test_pickle_should_reject_vanilla_pickle_copy_and_deepcopy(self):
