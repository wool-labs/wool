...
@pytest.mark.integration
def test_pairwise_scenarios_should_cover_all_pool_modes():
...
def _discovery_by_pool_mode():
    """Map each pool mode in the covering array to its discovery values."""
    classified: dict[PoolMode, set[DiscoveryFactory]] = {}
    for scenario in PAIRWISE_SCENARIOS:
        classified.setdefault(scenario.pool_mode, set()).add(scenario.discovery)
    return classified


@pytest.mark.integration
def test_pairwise_scenarios_should_partition_pool_modes_by_discovery():
    """Test the covering array splits pool modes cleanly by discovery.

    Given:
        The pairwise covering array generated by allpairspy plus its
        pinned backstop scenarios.
    When:
        The discovery values are collected per pool mode.
    Then:
        Every pool mode should carry at least one scenario and be
        either always NONE or never NONE — a mode that mixes both
        would mean the builder's discovery contract is unpinned for
        it.
    """
    # Act
    ...
    # Assert
    assert set(classified) == set(PoolMode)
    for pool_mode, discoveries in classified.items():
        assert discoveries, pool_mode
        assert (discoveries == {DiscoveryFactory.NONE}) or (
            DiscoveryFactory.NONE not in discoveries
        ), pool_mode
...
def test_scenarios_strategy_should_agree_with_the_covering_array(scenario):
    ...
    # Act
    discovery_is_none = scenario.discovery is DiscoveryFactory.NONE
...
@example(scenario=default_scenario(pool_mode=PoolMode.NESTED_RETIRED_IN_EPHEMERAL))
@given(scenario=scenarios_strategy())
