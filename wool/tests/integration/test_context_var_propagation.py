...
class TestWoolContextAcrossWorkers:
    ...
    async def test_seeded_context_dispatch_propagates_var_bindings(
        ...
        When:
            ...
            context's ``run``.
        Then:
            It should return the seed value — the wire context picks
        ...
        async def body():
            ...
            async with build_pool_from_scenario(scenario, credentials_map):
                ...
                # the routine's wire context is encoded from the seeded
...
class TestContextDecodeWarningAcrossWorkers:
    ...
    @pytest.mark.asyncio
    async def test_unpicklable_var_value_under_strict_mode_raises_error(
        self, credentials_map, retry_grpc_internal
    ):
        """Test caller-side strict mode raises ContextDecodeError.
        ...
        Then:
            It should raise a ``wool.ContextDecodeError`` aggregating
            ``wool.ContextDecodeWarning`` instances on
        ...
        async def body():
            ...
            assert all(
                isinstance(w, wool.ContextDecodeWarning) for w in warnings_list
            ), f"Expected only ContextDecodeWarning items, got {warnings_list!r}"
...
class TestWorkerSideContextDecodeFailure:
    @pytest.mark.asyncio
    async def test_worker_side_decode_failure_drops_var_and_dispatch_completes(
        self, credentials_map, retry_grpc_internal
    ):
        """Test a worker-side context decode failure degrades gracefully.
        ...
        Then:
            ...
            ``ContextDecodeWarning``, and the routine should still
...
class TestWorkerCrashMidDispatch:
    @pytest.mark.asyncio
    async def test_worker_crash_mid_dispatch_leaves_caller_context_intact(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestCancellationWithArmedContext:
    @pytest.mark.asyncio
    async def test_cancel_armed_dispatch_leaves_caller_context_intact(
...
class TestMidStreamContextDecodeFailure:
    @pytest.mark.asyncio
    async def test_mid_stream_decode_failure_drops_var_and_stream_continues(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestConcurrentDispatchesShareParentVar:
    @pytest.mark.asyncio
    async def test_concurrent_fan_out_reads_parent_set_var_without_contamination(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestAsyncGenSetAndResetAcrossYield:
    @pytest.mark.asyncio
    async def test_async_gen_set_then_reset_back_propagates_per_yield(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestCopyContextWidthAcrossWorkers:
    @pytest.mark.asyncio
    async def test_copy_context_enumerates_one_plus_n_after_dispatch(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestRoutineLookupErrorBackPropagation:
    @pytest.mark.asyncio
    async def test_get_on_unbound_default_less_var_surfaces_lookup_error(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestMultiWorkerFanOutWithContext:
    @pytest.mark.asyncio
    async def test_same_value_fans_out_to_distinct_workers_independently(
        self, credentials_map, retry_grpc_internal
    ):
...
class TestChainContentionAcrossDispatch:
    @pytest.mark.asyncio
    async def test_off_owner_thread_var_access_raises_chain_contention(
        self, credentials_map, retry_grpc_internal
    ):
    ...
    @pytest.mark.asyncio
    async def test_interleaved_async_gen_dispatches_on_a_shared_chain(
        self, credentials_map, retry_grpc_internal
    ):
        ...
        async def body():
            ...
            # Assert
            assert collected == [caller.chain_id.hex] * 6
