...
left unclosed when the loop that opened it closes. Their second subject
is partition isolation, on which see
`~wool.runtime.resourcepool.ResourcePool`: a lifecycle run on one loop
must leave every other loop's channels — a worker's own included —
exactly as it found them. They stand apart from the pairwise array
because the oracle here is what the pool holds after a successful
...
#: closed loop stranded there is a separate record from its channels'.
...
async def _records_after_stranding(
    caplog, factory, *, trigger=clear_channel_pool, discard_holds=False
):
    """Run ``factory()`` on a foreign loop and report what its discard drops.
    ...
    is what discards the closed loop's partition and drops whatever it
    ...
    since a read discards nothing and a clear would close what the caller
    is measuring. A sync or async trigger is equally accepted.

    The holds keep their own pool and their own registry, so a hold a
    closed loop stranded is reported only once a live loop enters
    `channel_pool_hold`: ``discard_holds`` takes and releases one before
...
class TestChannelPoolLifecycle:
    ...
    async def test___aexit___should_strand_no_channel_when_loop_stops_after_exit(
        ...
        When:
            ...
            nothing of its own to keep — which is what discards the
            closed loop's partition.
        Then:
    ...
    async def test_clear_channel_pool_should_warn_when_loop_stops_without_a_proxy(
        ...
        """Test a channel nobody closed is reported when its loop closes.
        ...
        When:
            This test's loop makes its next channel-pool operation,
            discarding the closed loop's partition.
        Then:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_should_keep_this_loops_entry_when_it_discards(
        self, started_worker, retry_grpc_internal, caplog
    ):
        """Test a discard drops the closed loop's entries and only those.
        ...
        When:
            ...
            discards without closing anything of its own.
        Then:
            It should report exactly one warning for the entry the
            closed loop stranded, and leave this loop's own entry
        ...
        async def strand():
            connection = WorkerConnection(worker.address)
            # Deliberately never closed: this is the entry the discard
            # has to find.
            await connection.idle()
    ...
    @pytest.mark.asyncio
    async def test_channel_pool_hold_should_report_a_stranded_hold_when_loop_closes(
        self, started_worker, caplog
    ):
        """Test a hold a closed loop never released is reported too.

        Given:
            A `wool.WorkerProxy` entered on an exit stack that is never
            closed, dispatched through, on another thread's loop that
            then closes — so the loop strands both the proxy's hold and
        ...
        Then:
            ...
            hold: separate pools with separate registries, each
            discarding only on an operation that reaches it.
        """
        # Arrange
        worker = await started_worker(LocalWorker())
        ...
        _, channel_records = await _records_after_stranding(
            caplog, strand, discard_holds=True
        )
    ...
    async def test_channel_pool_stats_should_keep_a_parked_stream_referenced(
        ...
        """Test a stream parked mid-dispatch survives a closed loop's discard.
