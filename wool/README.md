...
- **Handshake/authentication** (`UNAUTHENTICATED`, or `UNAVAILABLE` carrying TLS evidence) — surfaces as `HandshakeError`, a non-transient `RpcError` subclass with a typed `reason`. The load balancer evicts the worker and records the rejection; if the pool drains entirely on handshake failures it raises `AllWorkersUnauthenticated` (a `NoWorkersAvailable` subclass) instead of the bare condition.
