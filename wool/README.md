...
A failed TLS/mTLS handshake (wrong CA, identity mismatch, expired/rejected certificate, plaintext-vs-encrypted) surfaces as a `HandshakeError` — a non-transient `RpcError` subclass carrying a typed `reason`. Because a handshake failure is recoverable via credential rotation, the load balancer skips the worker without eviction (emitting a per-rejection warning log with the classified `reason`) so it retries on a later dispatch. When a dispatch cycle drains, the dispatcher raises the plain `NoWorkersAvailable`; the per-rejection logs make a fleet-wide credential misconfiguration diagnosable. (Surfacing the per-worker failures as a typed aggregate signal is planned as a follow-up.)
